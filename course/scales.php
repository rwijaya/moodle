...
if ($scaleid) {
    if ($scale = $DB->get_record("scale", array('id'=>$scaleid))) {
        if ($scale->courseid == 0 || $scale->courseid == $course->id) {
            ...
            echo html_writer::select($scalemenu, 'unused', '', '', array('id' => 'scaleunused'.$scaleid));
            echo "</center>";
...
if ($scales = $DB->get_records("scale", array("courseid"=>$course->id), "name ASC")) {
    ...
    foreach ($scales as $scale) {
        ...
        echo html_writer::select($scalemenu, 'unused', '', '', array('id' => 'courseunused' . $scale->id));
        echo "</center>";
        echo text_to_html($scale->description);
        echo $OUTPUT->box_end();
        echo "<hr />";
...
if ($scales = $DB->get_records("scale", array("courseid"=>0), "name ASC")) {
    ...
    foreach ($scales as $scale) {
        ...
        echo html_writer::select($scalemenu, 'unused', '', '', array('id' => 'sitescale' . $scale->id));
        echo "</center>";
        echo text_to_html($scale->description);
        echo $OUTPUT->box_end();
        echo "<hr />";
