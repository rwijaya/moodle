...
DIALOGUE = function(config) {
    ...
    config.notificationBase =
        Y.Node.create('<div class="'+CSS.BASE+'">')
              ...
              .append(Y.Node.create('<div id="'+id+'-content" class="'+CSS.BODY+' yui3-widget-bd"></div>'))
...
Y.extend(DIALOGUE, Y.Panel, {
    ...
    trapFocus : function() {
        var bb = this.get('boundingBox');
        Y.one('#'+bb.get('id')).on('keydown', function(e) {
            if (e.keyCode === 9) { // Tab key

                var dialogid = Y.one('#'+bb.get('id')).get('children').getAttribute('id');//this.get('children').slice(-1).item(0);
                //var dialogcontentid = Y.one('#'+dialogid).get('children').getAttribute('id');//this.get('children').slice(-1).item(0);

                var ids = [];//new Array();
                Y.one('#'+dialogid).all('*').each(function(n) {
                    if (n.get('id') !== '') {
                        ids.push(n.get('id'));
                    }
                });
                //  alert(ids.toSource());
                var target = Y.YUI2.util.Event.getTarget(e);
                var lastitem = ids[ids.length-1];
                if (lastitem === target.get('id')) { alert(target.get('id'));
                    Y.one('#'+ids[0]).focus();
                }
            }
        }, this);
        ...
        //FOCUSABLEITEMS[dialogid] = [];
        //this.focusableItems(dialogid, dialogcontentid);

        //var lastitem = FOCUSABLEITEMS[dialogid][FOCUSABLEITEMS[dialogid].length - 1];//alert(lastitem + '  ' + currentTarget);
        //if (lastitem === currentTarget){ //alert ('here');
        //    Y.one('#'+this.get('id')).focus();
        //}
    }
}, {
    NAME : DIALOGUE_NAME,
