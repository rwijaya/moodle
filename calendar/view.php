...
if (!empty($CFG->enablecalendarexport)) {
    ...
    if (isloggedin()) {
        $authtoken = sha1($USER->id . $USER->username . $USER->password . $CFG->calendar_exportsalt);
        $link = new moodle_url('/calendar/export_execute.php', array('preset_what'=>'all', 'preset_time'=>'recentupcoming', 'userid' => $USER->id, 'username'=>$USER->username, 'authtoken'=>$authtoken));
